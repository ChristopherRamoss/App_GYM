...
# CONFIG / NAV
# ══════════════════════════════════════════════════════════════════
inject_bottom_nav(active="estadisticas")
...
<p style="
        ...
        color:#666;
    ">
        Monitorea tu rendimiento, volumen, progreso y records personales
</p>
</div>
""", unsafe_allow_html=True)
...
df_todo = pd.read_sql_query(
    ...
    FROM entreno e
    LEFT JOIN sesiones s ON e.sesion_id = s.id
    ORDER BY e.fecha
    """,
    conn
)
...
with tab_resumen:
    ...
    if df_semana.empty:
        ...
        ">
            ...
            <p style="color:#fff;font-size:16px;font-weight:700;margin:12px 0 4px;">
                Sin entrenamientos esta semana
            </p>
            <p style="color:#666;font-size:13px;margin:0;">
                Completa una sesión para comenzar a generar estadísticas
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        c1.metric("Volumen semana", f"{vol_sem:,.0f} lb")
        c2.metric("Series totales", series_n)
        c3.metric("Días entrenados", dias_n)

        st.markdown("<div style='height:20px'></div>", unsafe_allow_html=True)

        # ── Músculos
        st.markdown("""
        <p style="font-size:13px;font-weight:700;color:#666;
        ...
        if not freq.empty:
            ...
            ).properties(height=max(180, len(freq)*28))
...
with tab_avanzado:
    ...
    if df_mes.empty:
        ...
        if vol_musculo:
            ...
            df_vm = pd.DataFrame(
                list(vol_musculo.items()),
                columns=["Músculo", "Volumen"]
        ...
        # ════════════════════════════════════════
        st.subheader("Frecuencia semanal")
        ...
        freq_prom = {
            m: len(dias) / n_semanas
            for m, dias in freq_sem.items()
        }

        if freq_prom:

            df_fp = pd.DataFrame(
                list(freq_prom.items()),
                columns=["Músculo", "Días/sem"]
            ...
            chart_fp = alt.Chart(df_fp).mark_bar(
                ...
                tooltip=[
                    "Músculo",
                    alt.Tooltip("Días/sem:Q", format=".1f")
                ]
            ).properties(height=220)
        ...
        if not intens.empty:
            ...
            chart_int = alt.Chart(intens).mark_bar(
                ...
                tooltip=[
                    "Ejercicio",
                    alt.Tooltip("Peso Prom (lb):Q", format=".1f")
                ]
            ).properties(height=max(200, len(intens)*28))
        ...
        # E. Peso vs Volumen
        # ════════════════════════════════════════
        st.subheader("Peso corporal vs Volumen")

        df_pc = pd.read_sql_query(
            "SELECT DATE(fecha) as dia, peso FROM peso_corporal ORDER BY fecha",
            conn
        )
        ...
        if not df_pc.empty and not df_vd.empty:
            ...
            base_p = alt.Chart(df_pc).encode(
                x=alt.X("dia:T", axis=alt.Axis(format="%d/%m"))
            )

            base_v = alt.Chart(df_vd).encode(
                x=alt.X("dia:T", axis=alt.Axis(format="%d/%m"))
            )

            linea = base_p.mark_line(
                color="#ef4444",
                strokeWidth=2,
                point=alt.OverlayMarkDef(
                    color="#ef4444",
            ...
            barras = base_v.mark_bar(
                color="#3b82f6",
                opacity=0.4,
                cornerRadiusTopLeft=4,
                cornerRadiusTopRight=4
            ).encode(
                y=alt.Y("vol:Q", title="Volumen")
            )

            st.altair_chart(
                alt.layer(barras, linea)
...
with tab_calendario:
    ...
    df_dias = pd.read_sql_query(
        """
        SELECT DATE(inicio) as dia, nombre_rutina
        FROM sesiones
    ...
    if not df_dias.empty:
        for _, r in df_dias.iterrows():
            try:
                ...
                dias_entrenados[d] = dias_entrenados.get(
                    d,
                    r['nombre_rutina']
                )
    ...
    <h3 style="
        ...
        font-weight:700;
    ">
        {meses_es[st.session_state.cal_mes-1]} {st.session_state.cal_anio}
    </h3>
    """, unsafe_allow_html=True)

    cal_grid = calendar.monthcalendar(
    ...
    for sem in cal_grid:
        ...
        for dn in sem:
            ...
            if dn == 0:
                ...
                if d_obj in dias_entrenados:
                    cls += " ent"
                    title = dias_entrenados[d_obj]
    ...
    mes_dias = [
        d for d in dias_entrenados
        if d.month == st.session_state.cal_mes
        and d.year == st.session_state.cal_anio
    ]
    ...
    if mes_dias:
        ...
        ">
            <p style="margin:0;color:#fff;font-weight:700;">
                {len(mes_dias)} días entrenados este mes
            </p>
        </div>
        """, unsafe_allow_html=True)
...
with tab_prs:
    ...
    if df_prs.empty:
        ...
        st.info("Completa algunos entrenamientos para ver tus records.")

    else:

        buscar = st.text_input(
            "Buscar ejercicio",
            placeholder="Ej: press, curl, squat..."
        )
        ...
        st.subheader("Top 10 ejercicios por PR")
        ...
        if not top10.empty:
            ...
            c10 = alt.Chart(top10).mark_bar(
                ...
                tooltip=[
                    "ejercicio",
                    alt.Tooltip("max_peso:Q", format=".1f")
                ]
